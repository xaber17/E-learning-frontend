...
export default function InputPotonganForm({ currentData, menu, action }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      potongan_lain: currentData?.potongan_lain || 0,
      ...
      <Grid container spacing={3}>
        <Grid item xs={12}>
          <Card sx={{ p: 3 }}>
            ...
            >
              <Typography variant="h5">Potongan</Typography>
              ...
              <RHFTextField
                ...
                label="Potongan Lain"
                placeholder="0.00"
                value={getValues('potongan_lain') <= 0 ? 0 : getValues('potongan_lain')}
                onChange={(event) => setValue('potongan_lain', Number(event.target.value))}
                InputLabelProps={{ shrink: true }}
                InputProps={{
                  startAdornment: <InputAdornment position="start">Rp</InputAdornment>,
                  type: 'number',
