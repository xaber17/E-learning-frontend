...
// List
import {
  FormVerifikasiPenggajianListHead,
  FormVerifikasiPenggajianListToolbar,
  FormVerifikasiPenggajianMoreMenu,
} from './list';

// ----------------------------------------------------------------------

InputVerifikasiPenggajianForm.propTypes = {
...
const TABLE_HEAD = [
  ...
  { id: 'tunjanganPerKehadiran', label: 'Tunjangan Per Kehadiran', alignRight: false },
...
export default function InputVerifikasiPenggajianForm({ isEdit, currentUser }) {
  const navigate = useNavigate();

  const { enqueueSnackbar } = useSnackbar();

  const VerifikasiPenggajianSchema = Yup.object().shape({
  ...
  const methods = useForm({
    resolver: yupResolver(VerifikasiPenggajianSchema),
    defaultValues,
  });
        ...
        <Grid item xs={12}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              >
                <LoadingButton sx={{ m: 1 }} type="submit" variant="contained" color="error" loading={isSubmitting}>
                  Tolak Verifikasi
                </LoadingButton>
