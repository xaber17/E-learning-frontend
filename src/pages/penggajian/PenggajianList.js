...
export default function PenggajianList() {
  ...
  useEffect(() => {
    ...
    try {
      if (choosenRole === 'stafinv') {
        dispatch(getPenggajianStafInv());
      } else if (choosenRole !== 'stafinv') {
        dispatch(getPenggajian());
      }
    } catch (e) {
      console.log(e);
    }
  ...
  const handleDetailPenggajian = (penggajian) => {
    ...
    if (choosenRole === 'stafpayroll') {
      if (penggajian.status_penggajian === '0' || penggajian.status_penggajian === '3') {
        window.localStorage.setItem('action', 'update');
      } else if (penggajian.status_penggajian !== '0' || penggajian.status_penggajian !== '3') {
        window.localStorage.setItem('action', 'detail');
      }
    } else if (choosenRole === 'manhrd') {
      if (penggajian.status_penggajian === '1') {
        window.localStorage.setItem('action', 'persetujuan');
      } else if (penggajian.status_penggajian !== '1') {
        window.localStorage.setItem('action', 'detail');
      }
    } else if (choosenRole === 'gm') {
      if (penggajian.status_penggajian === '2') {
        window.localStorage.setItem('action', 'verifikasi');
  ...
  let title = 'Penggajian';
  if (choosenRole === 'manhrd') {
    title = 'Persetujuan Penggajian';
  } else if (choosenRole === 'gm') {
    title = 'Verifikasi Penggajian';
  } else if (choosenRole === 'stafpayroll') {
    title = 'Penggajian';
  } else if (choosenRole === 'stafinv') {
  ...
  return (
    <Page title={title}>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        {choosenRole === 'stafpayroll' ? (
          <HeaderBreadcrumbs
            heading={title}
            links={[{ name: 'Dashboard', href: PATH_DASHBOARD.root }, { name: title }]}
            action={
              <Button
                onClick={() => handleCreatePenggajian()}
                variant="contained"
                component={RouterLink}
                to={PATH_DASHBOARD.penggajian.form}
